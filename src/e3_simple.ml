...
module AA : E3_core.TA with 
  ...
  and type tm = CC.tm
  and type sym = CC.sym
  and type nt_item = CC.nt_item
  and type tm_item = CC.tm_item
  and type sym_item = CC.sym_item
  and type sym_list = CC.sym_list
  and type item = CC.item
  and type set_todo_done = CC.set_todo_done
  and type map_blocked_key = CC.map_blocked_key
  and type map_complete_key = CC.map_complete_key
  and type map_sym_sym_int_int = CC.map_sym_sym_int_int
  and type map_tm_int = CC.map_tm_int
  and type ty_loop2 = CC.ty_loop2
  and type 'string ty_ctxt = 'string CC.ty_ctxt
= struct
